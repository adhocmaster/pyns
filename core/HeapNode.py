...
class HeapNode(SimpleNode):
    """Heap node works exactly as SimpleNode, with one exception: The queue is a heapq. So, two packets having the same arrivalTime will appear in the queue in FIFO style. 
    This is a required structure because the EventSimulator heapq schedules latest event to earlier than older events which have the same time.
